...
const ViewAnniversaries = () => {
  ...
  const fetchReminders = async (date) => {
    ...
    try {
      ...
      const response = await axios.get(
        `http://localhost:5000/api/dateAnniversaries?date=${formattedDate}`
      );
  ...
  return (
    <div className="anniversary-reminder">
      <h1>Anniversaries</h1>
        ...
        {reminders.length > 0 ? (
          reminders.map((reminder, index) => (
            <div key={index} className="reminder">
              ...
              <p>Time: {reminder.time ? reminder.time : "Not set"}</p> {/* Display time correctly */}
                  ...
                  <div className="popup-inner">
                    <form onSubmit={handleUpdateReminderSubmit}>
                      ...
                      <input
                        type="time"
                        name="time"
                        value={updateReminder.time || ""} // Ensure time is set
                        onChange={handleUpdateReminderChange}
                      />
