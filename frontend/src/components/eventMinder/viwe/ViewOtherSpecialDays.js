...
import "./ViewOtherSpecialDays.css";
...
const ViewOtherSpecialDays = () => {
  ...
  const fetchReminders = async (date) => {
    ...
    try {
      ...
      const response = await axios.get(`http://localhost:5000/api/dateOtherSpecialDays?date=${formattedDate}`);
  ...
  const handleUpdateReminderSubmit = async (event) => {
    ...
    try {
      await axios.put(`http://localhost:5000/api/reminders/${updateReminder._id}`, updateReminder);
      fetchReminders(selectedDate);
  ...
  return (
    <div className="other-special-day-reminder">
        ...
        {reminders.length > 0 ? (
          reminders.map((reminder) => (
            <div key={reminder._id} className="reminder">
              <h3>{reminder.name}'s Special Day</h3>
              ...
              <p>Notification Time: {reminder.time}</p>
